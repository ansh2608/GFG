...
public class Main {
    public ArrayList<ArrayList<Integer>> criticalConnections(int v, ArrayList<ArrayList<Integer>> adj) {
        ...
        for(ArrayList<Integer> a : ans) Collections.sort(a);
    ...
    private static void dfs(int node, int parent, int timer, int[] disc, int[] low, ArrayList<ArrayList<Integer>> ans, boolean[] visited, ArrayList<ArrayList<Integer>> list) {
        visited[node] = true;
        disc[node] = timer;
        low[node] = timer;
        timer++;
